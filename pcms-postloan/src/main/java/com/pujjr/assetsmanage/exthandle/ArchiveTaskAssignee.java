...
/**
 * 归档任务按照产品编码、经销商、进行任务分单
 * **/
@Service("ArchiveTaskAssignee")
public class ArchiveTaskAssignee implements ITaskAssigneeHandle {
	...
	public String handle(String assigneeParam, TaskEntity taskEntity) {
		...
		//获取经销商ID
		Apply apply = applyService.getApply(archiveTaskPo.getAppId());
		SysBranch sysBranch = sysBranchService.getSysBranch(null, apply.getCreateBranchCode());
		ProcessTaskUserBo assignee = this.getProcessTaskAccount(apply.getProductCode(),sysBranch.getId(), 0 ,assigneeParam, null);
	...
	private ProcessTaskUserBo getProcessTaskAccount(String productCode,String dealerId,int overdueDay,String workgroupId,List<String> candidateAccounts) {
		// TODO Auto-generated method stub
		...
		List<HashMap> listMatch = workgroupService.getCollectionTaskMatchRuleAccountList(productCode, dealerId, overdueDay, listGroup, candidateAccounts);
		if(listMatch.size()==0)
		{
			return null;
		}
